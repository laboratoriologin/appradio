...
namespace RadioPlayer.Common.Architecture
{
    ...
    public static class Util
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static ObservableCollection<ItemVO> CarregaGridPromocaoBahiaSulFM()
        {
            lstItemVO = new ObservableCollection<ItemVO>();

            if (DataSourceRadio.ListaItemPromocaoBahiaSulVO.Count > 0)
            {
               // foreach (var itemPromocao in DataSourceRadio.ListaItemPromocaoBahiaSulVO.GroupBy(i => i.Guid).Take(4).ToObservableCollection())
           
                foreach (var itemPromocao in DataSourceRadio.ListaItemPromocaoBahiaSulVO.GroupBy(i => i.Guid).ToObservableCollection())
                {
